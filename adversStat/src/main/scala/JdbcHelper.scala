...
object AdBlacklistDAO {
  ...
  def insertBatch(adBlacklists: Array[AdBlacklist]) {
    ...
    // 获取对象池单例对象
    val mySqlPool: GenericObjectPool[MySqlProxy] = CreateMySqlPool()
    // 从对象池中提取对象
...
object AdUserClickCountDAO {
  ...
  def updateBatch(adUserClickCounts: Array[AdUserClickCount]) {
    ...
    val insertAdUserClickCounts: ArrayBuffer[AdUserClickCount] = ArrayBuffer[AdUserClickCount]()
    val updateAdUserClickCounts: ArrayBuffer[AdUserClickCount] = ArrayBuffer[AdUserClickCount]()
  ...
  def findClickCountByMultiKey(date: String, userid: Long, adid: Long): Int = {
    ...
    val sql: String = "SELECT clickCount FROM ad_user_click_count " +
      "WHERE date=? " +
      "AND userid=? " +
      "AND adid=?"
...
object AdStatDAO {
  ...
  def updateBatch(adStats: Array[AdStat]) {
    ...
    // 区分开来哪些是要插入的，哪些是要更新的
    val insertAdStats: ArrayBuffer[AdStat] = ArrayBuffer[AdStat]()
    val updateAdStats: ArrayBuffer[AdStat] = ArrayBuffer[AdStat]()

    val selectSQL: String = "SELECT count(*) " +
      "FROM ad_stat " +
      "WHERE date=? " +
      "AND province=? " +
      "AND city=? " +
    ...
    for (adStat <- adStats) {
      ...
      val params: Array[Any] = Array[Any](adStat.date, adStat.province, adStat.city, adStat.adid)
    ...
    // 对于需要更新的数据，执行批量更新操作
    // 此处的UPDATE是进行覆盖
    val updateSQL: String = "UPDATE ad_stat SET clickCount=? " +
      "WHERE date=? " +
      "AND province=? " +
      "AND city=? " +
      "AND adid=?"
...
object AdProvinceTop3DAO {
  ...
  def updateBatch(adProvinceTop3s: Array[AdProvinceTop3]) {
    ...
    for (adProvinceTop3 <- adProvinceTop3s) {
      // 组合新key
      val key: String = adProvinceTop3.date + "_" + adProvinceTop3.province
...
object AdClickTrendDAO {
  ...
  def updateBatch(adClickTrends: Array[AdClickTrend]) {
    ...
    for (adClickTrend <- adClickTrends) {
      // 通过查询结果判断当前项时待插入还是待更新
      val params: Array[Any] = Array[Any](adClickTrend.date, adClickTrend.hour, adClickTrend.minute, adClickTrend.adid)
      client.executeQuery(selectSQL, params, new QueryCallback {
        override def process(rs: ResultSet): Unit = {
          if (rs.next() && rs.getInt(1) > 0) {
    ...
    // 执行批量更新操作
    // 此处的UPDATE是覆盖
    val updateSQL: String = "UPDATE ad_click_trend SET clickCount=? " +
      "WHERE date=? " +
      "AND hour=? " +
      "AND minute=? " +
      "AND adid=?"
